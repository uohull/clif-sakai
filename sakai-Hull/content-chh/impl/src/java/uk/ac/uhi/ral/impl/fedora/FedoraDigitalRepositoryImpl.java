...
public class FedoraDigitalRepositoryImpl implements DigitalRepository, HttpHttpsUrlConsumer {
  ...
  private static String FEDORA_WRITEABLE_COLLECTION_PID = "";
...
//CLIF  for AXIS2 test framework
  ...
  public void init(PropertyResourceBundle repoConfig) {
	synchronized(FedoraDigitalRepositoryImpl.class) { 	  
	    ...
	    FEDORA_WRITEABLE_COLLECTION_PID = repoConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_FEDORA_WRITEABLE_COLLECTION_PID);
  ...
  public String createFolder(DigitalItemInfo item) {  
	    ...
	    try {
		    ...
		    ownerProperty.setVALUE("fedoraAdmin");
...
private void buildNewObjectDataStreams(DigitalItemInfo item,
	...
	// /////////////////////////////////////////////////////////////////////////////////////////////////
	// CLIF Rights record for the digital object
	DatastreamType rightsDatastream = object.addNewDatastream();
	rightsDatastream.setID("rightsMetadata");
	rightsDatastream.setSTATE(StateType.A);
	rightsDatastream.setCONTROLGROUP(DatastreamType.CONTROLGROUP.M);	
	DatastreamVersionType rightsDatastreamVersion = rightsDatastream.addNewDatastreamVersion();
	rightsDatastreamVersion.setID("rightsMetadata.0");
	rightsDatastreamVersion.setMIMETYPE("text/xml");
	rightsDatastreamVersion.setLABEL("Rights metadata");
	XmlContentType rightsXmlContent = rightsDatastreamVersion.addNewXmlContent();
	
	Element rightsCollection = buildRightsXml(item, isFolder, rightsXmlContent);	    
	rightsXmlContent.getDomNode().appendChild(rightsCollection);		 	
	
	// /////////////////////////////////////////////////////////////////////////////////////////////////
	// CLIF content Metadata record for the digital object
	DatastreamType contentMDDatastream = object.addNewDatastream();
	contentMDDatastream.setID("contentMetadata");
	contentMDDatastream.setSTATE(StateType.A);
	contentMDDatastream.setCONTROLGROUP(DatastreamType.CONTROLGROUP.M);	
	DatastreamVersionType contentDatastreamVersion = contentMDDatastream.addNewDatastreamVersion();
	contentDatastreamVersion.setID("contentMetadata.0");
	contentDatastreamVersion.setMIMETYPE("text/xml");
	contentDatastreamVersion.setLABEL("content metadata");
	XmlContentType contentXmlContent = contentDatastreamVersion.addNewXmlContent();
	
	Element contentCollection = buildContentMDXml(item, isFolder, contentXmlContent);	    
	contentXmlContent.getDomNode().appendChild(contentCollection);
...
private Element buildRDFXml(DigitalItemInfo item, boolean isFolder,
	...
	contentModel.setAttributeNS("http://www.w3.org/1999/02/22-rdf-syntax-ns#", "resource", "info:fedora/hull-cModel:LTI");
...
private Element buildMODSXml(DigitalItemInfo item, boolean isFolder,
	...
	Element modsCollection = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "modsCollection");
	Element mods = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "mods");
	mods.setAttribute("version", "3.3");
	Element modsTitleInfo = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "titleInfo");
	Element modsTitle = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "title");
	...
	if(isFolder) {
		modsTypeOfResource = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "typeOfResource");
	...
	Element modsName = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "name");
	modsName.setAttribute("type", "personal");	    
	Element modsNamePart = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "namePart");
	...
	Element modsRole = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "role");
	Element modsRoleTerm = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "roleTerm");
	...
	if(!isFolder) {
		modsPhysicalDescription = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "physicalDescription");
		modsExtent = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "extent");
		modsInternetMediaType = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "internetMediaType");
		modsDigitalOrigin = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "digitalOrigin");
	...
	if(!isFolder) {
		Element modsGenre = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3","genre");
		Text modsGenreText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("Learning and teaching materials");	    	    
		modsGenre.appendChild(modsGenreText);
		mods.appendChild(modsGenre);
		
		Element modsLocation = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "location");		
		Element modsUrl1 = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "url");
		Text modsUrl1Text = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("http://hydra.hull.ac.uk/resources/" + ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getPid());		
		modsUrl1.setAttribute("usage", "primary display");
		modsUrl1.setAttribute("access", "object in context");
		modsUrl1.appendChild(modsUrl1Text);	
		
		Element modsUrl2 = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "url");
		modsUrl2.setAttribute("access", "raw object");
		Text modsUrl2Text = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("http://hydra.hull.ac.uk/assets/" + ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getPid() + "/content");	    	    
		modsUrl2.appendChild(modsUrl2Text);
		
		modsLocation.appendChild(modsUrl1);
		modsLocation.appendChild(modsUrl2);	
		mods.appendChild(modsLocation);

		Element modsAccessCondition = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "accessCondition");
		modsAccessCondition.setAttribute("type", "useAndReproduction");		
		Text modsAccessConditionText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode(String.format(repoConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_COPYRIGHT),(Calendar.getInstance().get(Calendar.YEAR))));
		modsAccessCondition.appendChild(modsAccessConditionText);
		mods.appendChild(modsAccessCondition);
		
		String publisher = item.getPublisher();
		if((publisher == null ) || (publisher.equals("")) || (publisher.endsWith("NOT_SET")))
				publisher = repoConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_PUBLISHER);

		Element modsoriginInfo = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "originInfo");
		Element modsPublisher = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "publisher");
		Text modsPublisherText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode(publisher);
		modsPublisher.appendChild(modsPublisherText);
		modsoriginInfo.appendChild(modsPublisher);
		mods.appendChild(modsoriginInfo);
		
		Element modsLanguage = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "language");
		Element modsLanguageTerm1 = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "languageTerm");
		modsLanguageTerm1.setAttribute("type", "text");	
		Text modsLanguageTerm1Text = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("English");
		modsLanguageTerm1.appendChild(modsLanguageTerm1Text);
		Element modsLanguageTerm2 = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "languageTerm");
		modsLanguageTerm2.setAttribute("type", "code");	
		modsLanguageTerm2.setAttribute("authority", "iso639-2b");
		Text modsLanguageTerm2Text = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("eng");
		modsLanguageTerm2.appendChild(modsLanguageTerm2Text);	
		modsLanguage.appendChild(modsLanguageTerm1);
		modsLanguage.appendChild(modsLanguageTerm2);
		mods.appendChild(modsLanguage);
		
		Element modsrecordInfo = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "recordInfo");
		Element modsrecordContentSource = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "recordContentSource");
		Text modsrecordContentSourceText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode(repoConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_PUBLISHER));
		modsrecordContentSource.appendChild(modsrecordContentSourceText);
		modsrecordInfo.appendChild(modsrecordContentSource);		
		Element modsrecordCreationDate = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "recordCreationDate");
		modsrecordCreationDate.setAttribute("encoding", "w3cdtf");	// leave empty tag
		modsrecordInfo.appendChild(modsrecordCreationDate);		
		String modifiedDate = item.getModifiedDate();
		if (modifiedDate != null && (modifiedDate.length()  > 13)) {			
			Element modsrecordChangeDate = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "recordChangeDate");
			modsrecordChangeDate.setAttribute("encoding", "w3cdtf");			
			try {
				SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
				Date modDate = sdf.parse(modifiedDate.trim().substring(0, 13));
				SimpleDateFormat xmlDF = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
				String xmlDate = xmlDF.format(modDate);
				Text modsrecordChangeDateText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode(xmlDate);		
				modsrecordChangeDate.appendChild(modsrecordChangeDateText);
			}
			catch(Exception e) {
				
				log.info(LOG_MARKER + "buildMODSXml:exception parsing lastModified date " + e.toString());
			}
			modsrecordInfo.appendChild(modsrecordChangeDate);
		}
		Element modslanguageOfCataloging = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "languageOfCataloging");
		Element modsLanguageTerm = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "languageTerm");
		modsLanguageTerm.setAttribute("authority", "iso639-2b");	
		Text modsLanguageTermText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode("eng");
		modsLanguageTerm.appendChild(modsLanguageTermText);
		modslanguageOfCataloging.appendChild(modsLanguageTerm);
		modsrecordInfo.appendChild(modslanguageOfCataloging);	
		
		String description = item.getDescription();
		if((description != null) && (description != "") && (!description.endsWith("NOT_SET")))
		{
			Element modsAbstract = modsXmlContent.getDomNode().getOwnerDocument().createElementNS("http://www.loc.gov/mods/v3.3", "abstract");
			modsAbstract.setAttribute("displayLabel", "Content description");
			Text modsAbstractText = modsXmlContent.getDomNode().getOwnerDocument().createTextNode(description);
			modsAbstract.appendChild(modsAbstractText);
			modsrecordInfo.appendChild(modsAbstract);			
		}
		mods.appendChild(modsrecordInfo);		
	}
...
//buildRightsXml
//builds the Rights metadata xml for a new object
private Element buildContentMDXml(DigitalItemInfo item, boolean isFolder,
		XmlContentType xmlContent) {
	
	Element contentMD = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.hull.ac.uk/schemas/contentMetadata/v1","contentMetadata");   
	contentMD.setAttribute("type", "text");		
	Element contentMDResource = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.hull.ac.uk/schemas/contentMetadata/v1", "resource");	
	contentMDResource.setAttribute("sequence", "1");
	contentMDResource.setAttribute("id", "Document");
	contentMDResource.setAttribute("displayLabel", "Document");	
	contentMDResource.setAttribute("objectID", ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getPid());	
	contentMDResource.setAttribute("dissID", "content");	
	contentMDResource.setAttribute("dissType", "genericContent/content");	

	
	Element contentMDFile = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.hull.ac.uk/schemas/contentMetadata/v1", "file");	
	contentMDFile.setAttribute("id", "content");	
	contentMDFile.setAttribute("format", getTypeOfResource(item.getTitle())); 
	contentMDFile.setAttribute("mimeType", getContentType(item.getTitle())); 
	contentMDFile.setAttribute("size", "" + item.getContentLength());
	
	Element contentMDLocation = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.hull.ac.uk/schemas/contentMetadata/v1", "location");	
	contentMDLocation.setAttribute("type", "url");
	Text contentMDLocationText = xmlContent.getDomNode().getOwnerDocument().createTextNode("http://hydra.hull.ac.uk/assets/"  + ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getPid() + "/genericContent/content");
	
	contentMDLocation.appendChild(contentMDLocationText);
	contentMDFile.appendChild(contentMDLocation);		
	contentMDResource.appendChild(contentMDFile);	
	contentMD.appendChild(contentMDResource);	
...
// buildRightsXml
// builds the Rights metadata xml for a new object
private Element buildRightsXml(DigitalItemInfo item, boolean isFolder,
		XmlContentType xmlContent) {
	
	Element rights = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","rightsMetadata");   
	Element rightsCopyright = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "copyright");	
	Element rightsHuman = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Text rightsHumanText = xmlContent.getDomNode().getOwnerDocument().createTextNode(String.format(repoConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_COPYRIGHT),(Calendar.getInstance().get(Calendar.YEAR))));
	rightsHuman.appendChild(rightsHumanText);
	Element rightsMachine = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");		
	rightsCopyright.appendChild(rightsHuman);
	rightsCopyright.appendChild(rightsMachine);
	
	Element rightsAccess = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","access"); 
	rightsAccess.setAttribute("type", "discover");	
	Element rightsHuman2 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Element rightsMachine2 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");
	Element rightsGroup1 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "group");
	Element rightsGroup2 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "group");
	Text rightsGroup1Text = xmlContent.getDomNode().getOwnerDocument().createTextNode("staff");
	Text rightsGroup2Text = xmlContent.getDomNode().getOwnerDocument().createTextNode("contentAccessTeam");
	rightsGroup1.appendChild(rightsGroup1Text);
	rightsGroup2.appendChild(rightsGroup2Text);
	rightsMachine2.appendChild(rightsGroup1);
	rightsMachine2.appendChild(rightsGroup2);	
	rightsAccess.appendChild(rightsMachine2);	
	rightsAccess.appendChild(rightsHuman2);
  
	Element rightsAccess2 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","access"); 
	rightsAccess2.setAttribute("type", "read");	
	Element rightsHuman22 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Element rightsMachine3 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");
	Element rightsGroup21 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "group");
	Element rightsGroup22 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "group");
	Text rightsGroup21Text = xmlContent.getDomNode().getOwnerDocument().createTextNode("staff");
	Text rightsGroup22Text = xmlContent.getDomNode().getOwnerDocument().createTextNode("contentAccessTeam");
	rightsGroup21.appendChild(rightsGroup21Text);
	rightsGroup22.appendChild(rightsGroup22Text);
	rightsMachine3.appendChild(rightsGroup21);
	rightsMachine3.appendChild(rightsGroup22);	
	rightsAccess2.appendChild(rightsMachine3);	
	rightsAccess2.appendChild(rightsHuman22);  
      
	Element rightsAccess3 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","access"); 
	rightsAccess3.setAttribute("type", "edit");	
	Element rightsHuman32 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Element rightsMachine4 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");
	Element rightsGroup31 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "group");
	Text rightsGroup31Text = xmlContent.getDomNode().getOwnerDocument().createTextNode("contentAccessTeam");
	rightsGroup31.appendChild(rightsGroup31Text);
	rightsMachine4.appendChild(rightsGroup31);
	rightsAccess3.appendChild(rightsMachine4);	
	rightsAccess3.appendChild(rightsHuman32);  
    
	Element rightsUse = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","use"); 	
	Element rightsHuman42 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Element rightsMachine5 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");
	rightsUse.appendChild(rightsMachine5);	
	rightsUse.appendChild(rightsHuman42);  
	
	Element rightsEmbargo = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1","embargo"); 	
	Element rightsHuman52 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "human");
	Element rightsMachine6 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://hydra-collab.stanford.edu/schemas/rightsMetadata/v1", "machine");
	rightsEmbargo.appendChild(rightsMachine6);	
	rightsEmbargo.appendChild(rightsHuman52);
  
	rights.appendChild(rightsCopyright);
	rights.appendChild(rightsAccess);
	rights.appendChild(rightsAccess2);
	rights.appendChild(rightsAccess3);
	rights.appendChild(rightsUse);
	rights.appendChild(rightsEmbargo);	
...
private Element buildDCXml(DigitalItemInfo item, boolean isFolder,
	...
	String modifiedDate = ((FedoraItemInfo)item).getModifiedDate(); 
	if (modifiedDate != null && (modifiedDate.length()  > 13)) {
		Element dcDate = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://purl.org/dc/elements/1.1/", "date");
		try {
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
			Date modDate = sdf.parse(modifiedDate.trim().substring(0, 13));
			SimpleDateFormat xmlDF = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
			String xmlDate = xmlDF.format(modDate);
			textNode = xmlContent.getDomNode().getOwnerDocument().createTextNode(xmlDate);				
			dcDate.appendChild(textNode);
			dc.appendChild(dcDate);
		}
		catch(Exception e) {
			
			log.info(LOG_MARKER + "buildDCXml:exception parsing lastModified date " + e.toString());
		}		
	}
	
	Element dcLang = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://purl.org/dc/elements/1.1/", "lang");
	textNode = xmlContent.getDomNode().getOwnerDocument().createTextNode("en");
	dcLang.appendChild(textNode);	
	...
	if(!isFolder) {
		...
		Element dcFormat2 = xmlContent.getDomNode().getOwnerDocument().createElementNS("http://purl.org/dc/elements/1.1/", "format");
		textNode = xmlContent.getDomNode().getOwnerDocument().createTextNode(getContentType("Filesize: " + (item.getContentLength() / 1000) + "KB"));
		dcFormat2.appendChild(textNode);
		dc.appendChild(dcFormat2);		
	}
	...
	if(!isFolder) {
		...
		Text dcTypeOfResourceText = xmlContent.getDomNode().getOwnerDocument().createTextNode("Learning and teaching materials");
  ...
  public boolean commitObject(DigitalItemInfo item) {
	...
	synchronized(FedoraDigitalRepositoryImpl.class) { 	  
		...
		try {
			...
			for(Iterator<String> i = Arrays.asList(identParts).iterator(); i.hasNext();) {
				if(((String)(i.next())).toLowerCase().equals(FEDORA_WRITEABLE_COLLECTION_PID.toLowerCase()) && i.hasNext()) {
					allowed = true;
					break;
				}
			}
			...
			if(!item.isCollection()) {  
			    ...
			    if ((currentResource != null) && ((newContent == null)||((item.getContentLength() == currentResource.getContentLength()))) && (((FedoraPrivateItemInfo)(item.getPrivateInfo())).getPid() != null) && (currentResource.getParentPid().equals(parentPid))) {		
			      ...
			      success = modifyObject(item, ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getMODsDatastreamID(),
		                  null, INLINE_UPDATE);
			      
			      if(success) {
			    	  success = modifyObject(item, ((FedoraPrivateItemInfo)(item.getPrivateInfo())).getDCDatastreamID(),
			                                 null, INLINE_UPDATE);
				...
				log.info(LOG_MARKER + "commitObject:attempt to modify a non-writeable collection was blocked"); 				
				return false;
			}
		}
		finally {
...
*/		
  ...
  public void gatherMoreResourceData(FedoraItemInfo item) {
	synchronized(FedoraDigitalRepositoryImpl.class) { 	  
	  try {
        ...
        item.setURL(repoConfig.getString(CONFIG_KEY_DISSEMINATION_ENDPOINT) + "/" + item.getIdentifier() + "/" + "content");
        ((FedoraPrivateItemInfo)(item.getPrivateInfo())).setContentDatastreamID("content");
        ((FedoraPrivateItemInfo)(item.getPrivateInfo())).setDCDatastreamID("DC");
        ((FedoraPrivateItemInfo)(item.getPrivateInfo())).setMODsDatastreamID("descMetadata");
	  }
