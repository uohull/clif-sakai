...
public class ContentHostingHandlerImplFedora implements ContentHostingHandlerFedora {
  ...
  private boolean actionAllowable(String targetId) {
	  
		String writeAbleCollectionPID = _rootConfig.getString(DigitalRepository.CONFIG_KEY_CLIF_FEDORA_WRITEABLE_COLLECTION_PID);
		boolean allowed = false;
		for(Iterator<String> i = Arrays.asList(targetId.split("/")).iterator(); i.hasNext();) {
			if(((String)(i.next())).toLowerCase().equals(writeAbleCollectionPID.toLowerCase()) && i.hasNext()) {
				allowed = true;
				break;
			}
		}
	...
	public String moveCollection(ContentCollectionEdit thisCollection, String new_folder_id) {
		if(actionAllowable(new_folder_id)) {
			synchronized(FedoraDigitalRepositoryImpl.class) {
