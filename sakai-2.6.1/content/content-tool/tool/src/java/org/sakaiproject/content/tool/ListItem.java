/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/content/tags/sakai-2.6.1/content-tool/tool/src/java/org/sakaiproject/content/tool/ListItem.java $
 * $Id: ListItem.java 61616 2009-05-01 04:08:28Z jimeng@umich.edu $
 ***********************************************************************************
 *
...
//				}
		...
		*/
		...
		try {
			...
			if(creator != null)
			{
				String creatorBy = creator.getDisplayName();
				if(creatorBy.equals(""))
					setCreatedBy(props.getProperty(ResourceProperties.PROP_CREATOR));   // user name format potentially from other systems e.g. Windows AD
				else
					setCreatedBy(creatorBy);
			}
			else
			{
				setCreatedBy(props.getProperty(ResourceProperties.PROP_CREATOR));   // user name format potentially from other systems e.g. Windows AD								
    	...
    	try{
    		String sUTDate;
    		Date d;
    	    DateFormat df = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.MEDIUM,new Locale("en", "US"));  // looks nicer than GB
    		
		    sUTDate = props.getProperty(ResourceProperties.PROP_MODIFIED_DATE); 		    		
		    XmlCalendar xmlCal = new XmlCalendar(sUTDate);
		    GregorianCalendar gregCal = new GregorianCalendar();
		    gregCal.setTime(xmlCal.getTime());	         
		    gregCal.setTimeZone(TimeZone.getTimeZone("GMT"));				      
		    d = gregCal.getTime();
	    	this.setModifiedTime(df.format(d));		    
    	}    	
	    catch (Exception ex){
