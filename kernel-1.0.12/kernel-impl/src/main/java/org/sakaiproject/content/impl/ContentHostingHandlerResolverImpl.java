...
import org.sakaiproject.content.api.ContentHostingHandlerFedora;
...
public class ContentHostingHandlerResolverImpl implements ContentHostingHandlerResolver
{
	...
	// CLIF  - TODO  check this method actually used ...if not delete from final code
	public ContentEntity getRealParentNoSelf( String id )
	{
		...
		if (lastSlash > 0)
		{
			...
			ContentEntity parentCE = null;			
			parentCE = storage.getCollection(parentId);
			ce = parentCE;
		}
	...
	public ContentEntity getVirtualEntity(ContentEntity ce, String finalId)
	{
		...
		if (chhbeanname != null && chhbeanname.length() > 0)
		{
			try
			{
				chh = (ContentHostingHandler) ComponentManager.get(chhbeanname);
				return chh.getVirtualContentEntity(ce, finalId);
	...
	public ContentEntity getVirtualChild(String finalId, ContentEntity ce, boolean exact)
	{
		...
		if (nextSlash == -1) {
			...
			if(storage instanceof FedoraContentStorage) {
				if(((FedoraContentStorage)storage).getStackMarkerDisabled() && (!finalId.endsWith(".properties")) && (!finalId.endsWith(".properties/")) && (finalId.contains(".properties/")) &&  finalId.endsWith("/") && (!nextId.endsWith("/")))
					 nextId = nextId + "/";   // CLIF
			}
	...
	public ContentCollection getCollectionNoRecursion( String id)
	{
		try
		{
			((FedoraContentStorage)(storage)).enableStackMarker();   // need to re-enable some subtle null recursion
			return getCollection( id );
		}
		finally
		{
			((FedoraContentStorage)(storage)).disableStackMarker();
		}
	}	
	...
	public ContentCollectionEdit putCollection( String id)
	{
		...
		if (ce != null)
		{
			...
			{
				ContentCollectionEdit cce = chh.getContentCollectionEdit(id);
				if(chh instanceof ContentHostingHandlerFedora) 
				{  // CLIF
					((FedoraContentStorage)storage).disableStackMarker();  // otherwise getRealParent(id) will fail;
					try
					{
						int lastSlash = id.lastIndexOf(Entity.SEPARATOR, id.length() - 2);
						if (lastSlash > 0)
						{
							String parentId = id.substring(0, lastSlash + 1);
							ContentEntity et = ((ContentHostingHandlerFedora)chh).commit(cce, id, storage.getCollection(parentId));
							cce.setVirtualContentEntity(et);							
						}					
					}
					finally
					{
						((FedoraContentStorage)storage).enableStackMarker();
					}
				}
				cce.setVirtualContentEntity(getVirtualChild(id, getRealParent(id), false).getVirtualContentEntity());
				return cce;
	...
	public Collection<String> getMemberCollectionIds(String collectionId)
	{
		...
		if (ce != null)
		{
			...
			if (chh != null)
			{				
				return chh.getMemberCollectionIds(ce.getVirtualContentEntity());
			}
		}
